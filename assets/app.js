...
function projectedThirds(){
  ...
  slots.slice().sort((a,b)=>a.cand.length-b.cand.length).forEach(s=>{
    let pick = top8.find(r=>s.cand.includes(r.group) && !used.has(r.group));
    pick = pick ? pick.group : s.cand.find(g=>!used.has(g));
    if(pick){ assign[s.code]=pick; used.add(pick); }
  });
  return (_projThirds = {assign, byGroup});
}
// Resolve a code to a projected real team name (or null if not yet projectable).
function projTeam(code){
  code = String(code);
  if(TEAMS[code]) return null;            // already a confirmed team
  let m;
  if(m = code.match(/^([12])([A-L])$/)){
    const r = standings("Group "+m[2])[m[1]==="1"?0:1];
    return (r && r.P>0) ? r.t : null;
  }
  if(code[0]==="3" && code.includes("/")){
    const {assign, byGroup} = projectedThirds();
    const g = assign[code]; return (g && byGroup[g]) ? byGroup[g] : null;
  }
  if(m = code.match(/^([WL])(\d+)$/)) return projWinner(+m[2], m[1]==="L");
  return null;
}
function projWinner(num, wantLoser){
  const mt = matchByNum(num); if(!mt) return null;
  if(mt.score && Array.isArray(mt.score.ft)){
    const [a,b] = mt.score.ft;
    const w = a>=b ? mt.team1 : mt.team2, l = a>=b ? mt.team2 : mt.team1;
    const pick = wantLoser ? l : w;
    return TEAMS[pick] ? pick : projTeam(pick);
  }
  const A = TEAMS[mt.team1] ? mt.team1 : projTeam(mt.team1);
  const B = TEAMS[mt.team2] ? mt.team2 : projTeam(mt.team2);
  if(!A || !B) return null;
  const strong = strengthOf(A) >= strengthOf(B) ? A : B, weak = strong===A ? B : A;
  return wantLoser ? weak : strong;
}
// Display info for one side of a bracket card.
function bracketTeam(code){
  if(TEAMS[code]){ const t=teamLabel(code); return {flag:t.flag, name:t.name, proj:false, ph:false}; }
  const p = projTeam(code);
  if(p){ const t=teamLabel(p); return {flag:t.flag, name:t.name, proj:true, ph:false}; }
  const t = teamLabel(code); return {flag:t.flag, name:t.name, proj:false, ph:true};
}
