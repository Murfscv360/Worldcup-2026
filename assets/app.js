...
async function fetchWC26(){
  try{
    const r = await fetchT(WC26 + "?_=" + Date.now(), 10000);
    if(!r.ok) throw 0;
    const j = await r.json();
    if(j && j.games && j.games.length){ LAST_WC_GAMES = j.games; return j.games; }
    return null;
  }catch(e){ return null; }   // CORS/offline/slow → caller reuses LAST_WC_GAMES
}
...
async function init(){
  ...
  // Refresh every 60s: base + live overlay.
  setInterval(async ()=>{ await loadData(); await loadLive(); setLive(); if(!editingSchedule()) render(); }, 60000);
