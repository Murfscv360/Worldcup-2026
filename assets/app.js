...
function viewSchedule(state){
  ...
  let html = `<div class="filters">
    ...
    <p class="hint">${list.length} match${list.length===1?"":"es"} · kickoff in ET &amp; your local time (${esc(localTZ)})</p>`;

  const keys = Object.keys(days).sort();
  if(!keys.length) html += `<div class="empty">No matches match your filters.</div>`;
  keys.forEach(k=>{
    const label = k==="TBD" ? "Date TBD" : dayLabelET(kickoffDate(days[k][0]));
    html += `<div class="daygroup"><div class="dayhead">${esc(label)}</div>${days[k].map(matchCard).join("")}</div>`;
  });
  return html;
}
...
function viewCommentary(){
  ...
  html += `<div class="sec-title"><h2>${st==="live"?"Live play-by-play":"Play-by-play"}</h2><span class="meta">minute-by-minute</span></div>`;
  const feed = matchFeed(sel);
  const icon = {ko:"🟢",goal:"⚽",var:"📺",ht:"⏸️",ft:"🏁",live:"🔴",play:"🎙️"};
  html += `<ul class="timeline">` + feed.slice().reverse().map(e=>`
      <li class="tl-${e.type}">
